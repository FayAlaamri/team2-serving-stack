...
def case(name, expected_status, payload=None, raw_body=None, headers=None):
    ...
    try:
        ...
        results.append(
            (name, False, f"EXCEPTION: {e}", expected)
        )
        return False, None
...
def run_cases():
    ...
    case(
        "'messages' is a string, not a list",
        422,
        payload={
            "model": MODEL,
            "messages": "hi",
    ...
    case(
        "minimal valid single-word message",
        200,
        payload={
            "model": MODEL,
            "messages": [
...
def run_concurrency_probe(n=2):
    ...
    print(
        f"\nconcurrency probe (n={n}): "
        f"wall={wall:.3f}s, "
        f"sum-of-individual={serial_estimate:.3f}s "
        f"({verdict})"
    )
...
def main():
    ...
    n_pass = sum(
        1 for _, ok, _, _ in results if ok
    )
